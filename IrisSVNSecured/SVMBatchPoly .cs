		...
		public class SecureSVC
		{
				...
				int[] weights, String kernel, double gamma, double coef0, ulong degree , int power)
			{
				...
				if (power <= 20)
				{
					...
					parms.CoeffModulus = CoeffModulus.Create(polyModulusDegree, new int[] { 40, 20 ,20 , 20 , 20 , 20 , 40 });
				}
				else
				{
			...
			public int Predict(double[] features, bool useRelinearizeInplace,bool useReScale)
			{
				...
				for (int i = 0; i < numOfrows; i++)
				{
					...
					if(kernel == Kernel.Poly)
					{
						...
						encoder.Encode(coef0, kernels[i].Scale, coef0Plaintext);
                        if (useReScale)
						{
							ParmsId lastParmsId = kernels[i].ParmsId;
							evaluator.ModSwitchToInplace(coef0Plaintext, lastParmsId);
						}

						//kernels[i].Scale = coef0Plaintext.Scale;

                        evaluator.AddPlainInplace(kernels[i], coef0Plaintext);
                        PrintScale(kernels[i], "2.7  kernels" + i);
		...
		//	Console.WriteLine(max);
			...
			)
			{
				...
				for (int i = 0; i < numOfRows; i++)
				{
					...
					timePredictSum.Stop();
					file.WriteLine($"SecureSVC estimation{i} is : {estimation} ");
					Console.WriteLine($"SecureSVC estimation{i} is : {estimation} ");
					Console.WriteLine($"$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$ \n\n");
				}
