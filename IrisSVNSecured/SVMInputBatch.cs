...
namespace IrisSVNSecured
{
    ...
    //            if (result[0] > 0)
		...
		//	Console.WriteLine(max);
            ...
            await RunPrediction(scale, useRelinearizeInplace, useReScale, numOfRows, processorCount, machines, vectors_mashroom, features, results, tasks);
		...
		private static async Task RunPrediction(int scale, bool useRelinearizeInplace, bool useReScale, int numOfRows,
			int processorCount, SecureSVC[] machines, double[][] vectors_mashroom, double[][] features, Result[] results,
			Task[] tasks)
		{
			using (System.IO.StreamWriter file =
				new System.IO.StreamWriter(
				...
				for (int i = -1; i < numOfRows-1;)
				{
					for (int j = 0 ; j < processorCount ; j++)
					{
						...
						for (int k = 0; k < batchSize * featureSize && i < numOfRows-1; k++)
						{
                            ...
                            if (index0 < vectors_mashroom[0].Length)
							{
